...
description: >
  ...
  TRIGGERS: "viết content KFSP", "viết bài fanpage KFSP", "biến câu chuyện này thành content KFSP",
  "content fit KFSP không", "ý chính + người đọc nhận được + liên kết KFSP", "idea fit", "viết theo style KFSP",
  "giọng KFSP", "đúng phong cách KFSP", "viết lại đoạn này cho KFSP", "content cho mẫu hình / chỉ báo / TA KFSP".
  Trigger khi user muốn TẠO content organic KFSP (fanpage/MXH) từ 1 nguồn hoặc 1 chủ đề.
  SKILL NÀY HOẠT ĐỘNG NHƯ MỘT "MASTER AGENT". AI phải tự động tải và đọc các reference cần thiết để hoàn tất quy trình 6 bước từ A-Z mà không yêu cầu user phải trigger thêm skill khác. KHÔNG trigger cho:
  script video TikTok (→ mkt-content-script / video-kfsp), ads copy Meta/TikTok (→ mkt-ads-copy), email (→ email-mkt).
---
...
### Bước 4 — Viết Content hoàn chỉnh & Áp giọng theo kênh
Viết nội dung hoàn chỉnh bám sát 100% dàn ý đã chốt, **không tự động phân đoạn hay đẻ thêm ý**. Áp giọng theo reference:
- Fanpage Facebook KFSP → **`reference/style-fanpage.md`** (tít CAPS 2 vế · đánh số · xưng "bạn" · CTA mẫu · brand spine).
- Kênh khác → xem `reference/style-<kênh>.md`.
