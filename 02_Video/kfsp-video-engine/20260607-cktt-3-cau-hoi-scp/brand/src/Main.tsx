...
// s12: app filter screenshot (BRAND only)
const SceneAppFilter: React.FC<{ f: number }> = ({ f }) => {
  const s = sp(f, 4, { damping: 18, stiffness: 120 });
  return (
    <Base>
      <Title f={f} lines={["Bộ lọc làm thay bạn", "trong vài giây"]} size={56} top={232} />
      <Img src={staticFile("app-boloc.jpg")} style={{ position: "absolute", top: 372, left: "50%",
        transform: `translateX(-50%) scale(${0.94 + s * 0.06})`, height: 940, borderRadius: 26,
        border: "2px solid rgba(123,58,236,.55)", boxShadow: "0 22px 64px rgba(0,0,0,.62)", opacity: s }} />
    </Base>
  );
};
...
const SCENES: { a: string; b: string | null; el: (f: number) => React.ReactNode }[] = [
  ...
  { a: "s11", b: "s12", el: (f) => <SceneRecap f={f} /> },
  { a: "s12", b: "s13b", el: (f) => <SceneAppFilter f={f} /> },
  { a: "s13b", b: null, el: (f) => <SceneCTA f={f} /> },
];
