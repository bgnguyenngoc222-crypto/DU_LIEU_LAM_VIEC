  ...
  return (
    <NavyBase>
      ...
      <div style={{ position: "absolute", top: 820, left: "50%", transform: `translateX(-50%) scale(${0.6 + fb * 0.4})`,
        opacity: fb, background: PUR, color: "#fff", fontSize: 46, fontWeight: 800, padding: "22px 60px",
        borderRadius: 44, whiteSpace: "nowrap", boxShadow: "0 10px 30px rgba(123,58,236,.5)" }}>Trải nghiệm KFSP</div>
...
const SFX_EVENTS: Sfx[] = (() => {
  ...
  ev.push({ f: F(ss("s01")) + 3, src: "whoosh.mp3", v: 0.14, d: 17 });
  ev.push({ f: F(ss("s02")) + 3, src: "whoosh.mp3", v: 0.14, d: 17 });
  ev.push({ f: F(ss("s03")) + 30, src: "pop.mp3", v: 0.2, d: 3 });
  ev.push({ f: F(ss("s03")) + 70, src: "pop.mp3", v: 0.2, d: 3 });
  ev.push({ f: F(ss("s05")) + 40, src: "pop.mp3", v: 0.18, d: 3 });
  ev.push({ f: F(ss("s12")) + F(1.2), src: "click.mp3", v: 0.24, d: 2 });
  ev.push({ f: F(ss("s13")) + F(3.5), src: "ting.mp3", v: 0.26, d: 11 });
  return ev;
})();
...
const Logo: React.FC = () => (
  <AbsoluteFill style={{ alignItems: "center", justifyContent: "flex-start", pointerEvents: "none" }}>
    <div style={{ position: "absolute", top: 70, display: "flex", alignItems: "center", gap: 10 }}>
      <Img src={staticFile("logo-kfsp.png")} style={{ width: 54, height: 54, objectFit: "contain",
        filter: "brightness(0) invert(1) drop-shadow(0 2px 6px rgba(0,0,0,.55))" }} />
      <div style={{ fontFamily: FF, fontSize: 30, fontWeight: 800, letterSpacing: 4, color: "#fff",
        textShadow: "0 2px 8px rgba(0,0,0,.55)" }}>KFSP</div>
    </div>
  </AbsoluteFill>
);
